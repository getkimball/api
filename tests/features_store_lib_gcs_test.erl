...
read_test() ->
    ...
    ?assertEqual(1, meck:num_calls(enenra, get_object_contents, [?BUCKET, ExpectedPath, ?ENENRA_CREDENTIALS])),
...
load() ->
    ...
    ok = meck:expect(application, get_env, [{[features, gcs_bucket], {ok, ?BUCKET}},
